...
class ValidateSlug
{
     ...
     *
     * A grid's pivot table is named after the stream table
     * and the field slug, so a slug that resolves to an
     * existing table would replace it.
     *
     * @param  string $value
    ...
    public function handle(
        ...
        foreach ($this->targets($builder, $streams) as $stream) {
            ...
            if ($schema->hasColumns($table, ['entry_id', 'entry_type', 'related_id', 'sort_order'])) {
                continue;
            }
